...
export default function TacticalSidebar({ isOpen, onClose }) {
    const { user } = useAuth();
    ...
    useEffect(() => {
        ...
        window.addEventListener("toggle-tactical-sidebar", handleToggle);
        window.addEventListener("close-tactical-sidebar", handleClose);
        return () => {
            window.removeEventListener("toggle-tactical-sidebar", handleToggle);
            window.removeEventListener("close-tactical-sidebar", handleClose);
        };
    }, []);

    const isDrawerActive = isOpen !== undefined ? isOpen : mobileOpen;
    ...
    return (
        <>
            ...
            <aside
                className={`fixed left-0 top-0 h-full w-64 bg-surface-container-lowest z-50 flex flex-col justify-between shadow-2xl border-r border-surface-container-high transition-transform duration-300 ease-in-out ${
                    isDrawerActive ? "translate-x-0" : "-translate-x-full md:translate-x-0"
                }`}
            >
                <div className="flex flex-col flex-1 min-h-0">
                    {/* Brand Header */}
                    <div className="h-16 px-space-md flex items-center justify-between bg-surface-container-lowest border-b border-surface-container-high/60">
